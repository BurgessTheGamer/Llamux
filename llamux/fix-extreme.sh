#!/bin/bash
#
# Fix the EXTREME build properly
#
...
# Now make proper changes with correct types
cat > extreme.patch << 'EOF'
--- main.c.orig
+++ main.c
@@ -227,7 +227,7 @@
     } else {
...
+        size_t max_tensor_size = 256ULL * 1024ULL * 1024ULL; /* 256MB limit */
         ...
         pr_info("🦙 Llamux: Loading model tensors (limited to %zu MB)...\n", 
EOF

patch -p0 < extreme.patch

echo "Building fixed EXTREME version..."
make clean && make

echo "Done! Now with 512MB allocation (more reasonable for vmalloc)"